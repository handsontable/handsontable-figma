...
The tool is a three-stage pipeline over a single in-memory object (`themeVariables`). All stages are pure transforms; filesystem I/O is confined to `index.js` and `utils/helpers/fileSystem.js`.

```
tokens.json ──► [Stage 1: themeProcessing] ──► themeVariables ──┬──► [Stage 2: jsGeneration]  ──► output/variables/**
                                                                └──► [Stage 3: cssGeneration] ──► output/css/**
```

## Stage 1 — Reference resolution (`utils/themeProcessing.js`)
...
```js
{
  ...
  tokens:  { [themeName]: { /* theme tokens, filtered by tokensKeys.js */ } },
}
```
...
Iterates `tokensKeys.js` in order, looks up each key recursively in the theme's value tree, and runs `processTokenValue`. Keys not found in the tree are silently skipped. This is the allow-list that defines the public surface of a theme.

## Stage 2 — JS serialization (`utils/jsGeneration.js`)

Entry point: `writeJsThemeFiles(themeVariables)`. Writes:

- `output/variables/sizing.js` and `density.js` — flat modules shared by all themes.
- `output/variables/colors/<name>.js` and `tokens/<name>.js` — one file per theme.
- `output/variables/icons/<name>.js` — one per entry in `ICONS_SET`.
- `output/variables/helpers/iconsMap.js` — copied verbatim from `utils/helpers/iconsMap.js`.
...
## Stage 3 — CSS serialization (`utils/cssGeneration.js`)
...
- `tokensKeys.js` as an explicit allow-list means adding Figma tokens doesn't accidentally leak them into the public theme API.
